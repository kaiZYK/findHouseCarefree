  ...
  if (telBool) {
    ...
    if (authCodeBtn.disabled) {
      ...
      setTimeout(function () {
        ...
        alert(
          `您修改密码的验证码为：${randomNumber}，请您尽快修改密码，请不要把验证码泄露给其他人，如非本人请勿操作！`
        );
        // 成功弹出验证码之后验证码次数+1
        indexAuthCode++;
      }, 3000);
...
password1.addEventListener("blur", function () {
  var reg = /^[A-z0-9]{6,}$/;
  var bool = reg.test(password1.value);
  ...
  if (password2 != "") {
    var reg = /^[A-z0-9]{6,}$/;
    var bool = reg.test(password2.value);
    ...
    if (password2.value == "") {
      ...
      if (bool) {
        ...
        if (password1.value == password2.value) {
          ...
          spans[3].innerHTML = "两次输入密码一致";
          // 密码布尔值返回正确
          passwordBool = true;
        } else {
          spans[3].className = "error";
          spans[3].innerHTML = "两次输入密码不一致";
          // 密码布尔值返回错误
          passwordBool = false;
        }
      } else {
        spans[3].className = "error";
        spans[3].innerHTML = "密码格式错误";
        // 密码布尔值返回错误
        passwordBool = false;
      }
    }
...
password2.addEventListener("focus", function () {
  spans[3].className = "";
  spans[3].innerHTML = "请输入字母，数子的密码，至少为6位";
});
